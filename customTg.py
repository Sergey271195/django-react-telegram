...
def bot_message_decorator(reply_message = None):
    def method_decorator(method_to_decorate):
        def wrapper(self, request_body, user_id, **kwargs):
            message = request_body.get('message')
            if message:
                user_id = message['from'].get('id')
                text = message.get('text')
                if not reply_message:
                    return method_to_decorate(self, request_body, user_id, **kwargs)
...
class TelegramBot():
    ...
    def sendLocation(self, user_id, latitude, longitude):
        send_location_url = os.path.join(self.url, 'sendLocation')
        telegram_request = requests.post(send_location_url, data = {'chat_id': user_id, 'latitude': latitude, 'longitude': longitude})
        response = telegram_request.json()
        return response
